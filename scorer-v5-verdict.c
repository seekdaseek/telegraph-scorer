...
float rank_answer(u32 q_ptr, u32 q_len, u32 gt_ptr, u32 gt_len, u32 ma_ptr, u32 ma_len) {
  ...
  if (short_verdict && gt_is_polar)
    ...
    // CONTAINMENT, NOT F1. gt "news" against "The site is categorised as news."
    // has recall 1.0 but precision 1/3, so the F1-blended text score was 0.75
    // and the map dropped a PERFECT answer to 0.690. A one-word verdict is
    // answered in a sentence by definition - the extra words are the sentence,
    // not noise - so only recall can be the signal here.
    base = text_recall(gbuf, gn, mbuf, mn);
  else base = (num < 0.0) ? txt : (0.85 * num + 0.15 * txt);
