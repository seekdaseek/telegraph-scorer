...
#define MAXN 8192
// LATIN DIACRITIC FOLDING. A translation with its accents stripped is still a
// correct translation - "Como estas" for "¿Cómo estás?", "prosze" for "proszę" -
// but a byte comparison rates it 0.0000 and ties it with a wrong answer. That
// diacritic-drop class was the ONLY one where the champion out-scored this
// scorer on a deliberately hard corpus (0.055 vs 0.000).
//
// Folding is confined to two UTF-8 ranges whose mapping to ASCII is
// unambiguous: Latin-1 Supplement (0xC3 0x80-0xBF) and Latin Extended-A
// (0xC4/0xC5). Cyrillic, Greek, CJK, Hangul and Arabic are NOT in those ranges
// and pass through byte-for-byte untouched, so the UTF-8 self-match fix is
// unaffected - which the test corpus re-checks across all of them.
static u8 fold2(u8 b1, u8 b2) {
  if (b1 == 0xC3) {
    static const char *T =
      "aaaaaa" "c" "eeee" "iiii" "d" "n" "ooooo" "x" "o" "uuuu" "y" "ps"
      "aaaaaa" "c" "eeee" "iiii" "d" "n" "ooooo" "/" "o" "uuuu" "y" "py";
    u32 i = (u32)(b2 - 0x80);
    if (i < 64) { char c = T[i]; if (c >= 'a' && c <= 'z') return (u8)c; }
    return 0;
  }
  if (b1 == 0xC4 || b1 == 0xC5) {
    // Latin Extended-A is laid out in upper/lower pairs by base letter.
    static const char *A = "aaaaaaccccccccddddeeeeeeeeeegggggggghhhhiiiiiiiiiiiijjkkkllllllllll";
    static const char *B = "llnnnnnnnnnooooooooeerrrrrrssssssssttttttuuuuuuuuuuuuwwyyyzzzzzzs";
    u32 i = (u32)(b2 - 0x80);
    if (b1 == 0xC4 && i < 64) return (u8)A[i];
    if (b1 == 0xC5 && i < 64) return (u8)B[i];
  }
  return 0;
}
...
static u32 normalize(const u8 *s, u32 n, u8 *out) {
  ...
  for (u32 i = 0; i < n && o < MAXN - 1; i++) {
    ...
    // Fold a Latin accent to its base letter; every other high byte is content
    // and passes through unchanged.
    if (c >= 0xC3 && c <= 0xC5 && i + 1 < n) {
      u8 f = fold2(c, s[i + 1]);
      if (f) { out[o++] = f; prev_space = 0; i++; continue; }
    }
